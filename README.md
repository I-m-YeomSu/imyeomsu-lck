...
- EKS를 도입하면 처음에 초기 작업은 어려울 수 있지만, 작성해둔 YAML 파일을 재사용하여 클러스터를 관리할 수 있어서 개발자들은 버전이나 클러스터 관리 문제를 신경 쓰지 않고도 응용 프로그램의 개발과 배포에 집중할 수 있습니다. 또한, 컨테이너의 개수가 많아져도 개발자들이 직접적으로 컨테이너를 관리하는 부담이 줄어들어서 더욱 수월하게 작업을 진행할 수 있다고 판단해 EKS 도입을 진행했습니다.
- AWS는 완전 관리형 Kubernetes 서비스인 Amazon EKS(Managed Kubernetes Service)를 제공하고 있어서 개발자들이 별도의 관리 요소나 커스터마이징을 신경 쓰지 않아도 됩니다. EKS는 고가용성과 확장성을 제공하며, 항상 최신 버전의 Kubernetes를 사용할 수 있도록 지원합니다. 또한, AWS가 서비스의 유지 및 업데이트를 거의 강제적으로 관리해주기 때문에 애플리케이션이 안정적으로 서빙되고 유지보수가 용이하다고 판단해 EKS 도입을 진행했습니다.
- 우리 팀이 고민하고 있던 부분의 최적의 해결 방안이라고 생각이 들었고 이를 통한 실제 고가용성을 고려한 프로젝트 역시 잘 마무리 하였습니다.
## CI/CD
### 1. CI/CD Flow
